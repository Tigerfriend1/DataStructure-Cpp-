...
friend ostream &operator<<(ostream&, Deque<T>&);
};
...
void Deque<T>::push_back(T const& x){
    if (deque[capacity-1] !=""){ //emtpy size is 0
        //index[0] is not empty -> should extend array!
        //capacity*2 and copy
        T* newDeque= new T[capacity*2];
...
int main(){
    Deque<string> d; // 안되면 string으로 해보자.
    for (int i=0; i<10; i++){
        d.push_front("a");
        d.push_back("b");
    }
    ...
    while (!d.IsEmpty()){
        ...
        cout<<d;
        cout<<endl<<"Front_pop is : "<<d.Front()<<endl;
        d.pop_front();
